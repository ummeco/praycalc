 ...
 * Constraints: MUST use /v1/auth/* endpoint forms (NOT a bespoke /token route).
...
export async function signup(
  ...
  const data = await postJson<HasuraAuthSignupResponse>('/v1/auth/signup/email-password', {
    email,
    password,
...
export async function signin(email: string, password: string): Promise<AuthSession> {
  const data = await postJson<HasuraAuthSigninResponse>('/v1/auth/signin/email-password', {
    email,
    password,
  });
  return data.session;
...
export async function refreshToken(refreshTokenValue: string): Promise<HasuraAuthRefreshResponse> {
  return postJson<HasuraAuthRefreshResponse>('/v1/auth/token', {
