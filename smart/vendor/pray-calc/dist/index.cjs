...
__export(index_exports, {
  ANGLE_MAX: () => ANGLE_MAX,
  ANGLE_MIN: () => ANGLE_MIN,
  DHUHR_OFFSET_MINUTES: () => DHUHR_OFFSET_MINUTES,
  METHODS: () => METHODS,
  calcTimes: () => calcTimes,
  calcTimesAll: () => calcTimesAll,
  getAngles: () => getAngles,
  getAsr: () => getAsr,
  getMidnight: () => getMidnight,
  getMscFajr: () => getMscFajr,
  getMscIsha: () => getMscIsha,
  getQiyam: () => getQiyam,
  getTimes: () => getTimes,
  getTimesAll: () => getTimesAll,
...
// src/getTimes.ts
var import_nrel_spa = require("nrel-spa");

// src/constants.ts
var DEG = Math.PI / 180;
var DHUHR_OFFSET_MINUTES = 2.5;
var ANGLE_MIN = 10;
var ANGLE_MAX = 22;
...
function computeAngles(date, lat, lng, elevation = 0, temperature = 15, pressure = 1013.25) {
...
// src/getMidnight.ts
function getMidnight(maghribTime, endTime) {
...
// src/validate.ts
function validateInputs(lat, lng, tz, elevation) {
  ...
  if (tz !== void 0 && (!Number.isFinite(tz) || tz < -14 || tz > 14)) {
    throw new RangeError(`timezone offset must be between -14 and 14, got ${tz}`);
  }
  if (elevation !== void 0 && (!Number.isFinite(elevation) || elevation < -500)) {
...
function getTimes(date, lat, lng, tz = -date.getTimezoneOffset() / 60, elevation = 0, temperature = 15, pressure = 1013.25, hanafi = false) {
  ...
  const spaData = (0, import_nrel_spa.getSpa)(date, lat, lng, tz, spaOpts, [fajrZenith, ishaZenith]);
...
// src/calcTimes.ts
var import_nrel_spa2 = require("nrel-spa");
function calcTimes(date, lat, lng, tz = -date.getTimezoneOffset() / 60, elevation = 0, temperature = 15, pressure = 1013.25, hanafi = false) {
  ...
  return {
    Qiyam: (0, import_nrel_spa2.formatTime)(raw.Qiyam),
    Fajr: (0, import_nrel_spa2.formatTime)(raw.Fajr),
    Sunrise: (0, import_nrel_spa2.formatTime)(raw.Sunrise),
    Noon: (0, import_nrel_spa2.formatTime)(raw.Noon),
    Dhuhr: (0, import_nrel_spa2.formatTime)(raw.Dhuhr),
    Asr: (0, import_nrel_spa2.formatTime)(raw.Asr),
    Maghrib: (0, import_nrel_spa2.formatTime)(raw.Maghrib),
    Isha: (0, import_nrel_spa2.formatTime)(raw.Isha),
    Midnight: (0, import_nrel_spa2.formatTime)(raw.Midnight),
    angles: raw.angles
  };
}

// src/getTimesAll.ts
var import_nrel_spa3 = require("nrel-spa");
var METHODS = [
  {
    id: "UOIF",
...
function getTimesAll(date, lat, lng, tz = -date.getTimezoneOffset() / 60, elevation = 0, temperature = 15, pressure = 1013.25, hanafi = false) {
  ...
  const spaOpts = { elevation, temperature, pressure };
  const spaData = (0, import_nrel_spa3.getSpa)(date, lat, lng, tz, spaOpts, allZeniths);
  const fajrTime = spaData.angles[0].sunrise;
  const sunriseTime = spaData.sunrise;
  const noonTime = spaData.solarNoon;
  const maghribTime = spaData.sunset;
...
// src/calcTimesAll.ts
var import_nrel_spa4 = require("nrel-spa");
function calcTimesAll(date, lat, lng, tz = -date.getTimezoneOffset() / 60, elevation = 0, temperature = 15, pressure = 1013.25, hanafi = false) {
  ...
  for (const [id, [fajr, isha]] of Object.entries(raw.Methods)) {
    Methods[id] = [(0, import_nrel_spa4.formatTime)(fajr), (0, import_nrel_spa4.formatTime)(isha)];
  }
  return {
    Qiyam: (0, import_nrel_spa4.formatTime)(raw.Qiyam),
    Fajr: (0, import_nrel_spa4.formatTime)(raw.Fajr),
    Sunrise: (0, import_nrel_spa4.formatTime)(raw.Sunrise),
    Noon: (0, import_nrel_spa4.formatTime)(raw.Noon),
    Dhuhr: (0, import_nrel_spa4.formatTime)(raw.Dhuhr),
    Asr: (0, import_nrel_spa4.formatTime)(raw.Asr),
    Maghrib: (0, import_nrel_spa4.formatTime)(raw.Maghrib),
    Isha: (0, import_nrel_spa4.formatTime)(raw.Isha),
    Midnight: (0, import_nrel_spa4.formatTime)(raw.Midnight),
    angles: raw.angles,
    Methods
  };
}
