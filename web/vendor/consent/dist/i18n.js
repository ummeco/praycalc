...
const ar = {
    ...
    preferences: {
        ...
        marketing: 'تسويق',
        marketingDesc: 'السماح بالمحتوى من جهات خارجية (مثل مقاطع فيديو يوتيوب) والإعلانات ذات الصلة. معطّلة افتراضياً.',
    },
};
const MESSAGES = {
    en,
...
const SCAFFOLD_LOCALES = ['id', 'ur', 'bn'];
export function getMessages(locale) {
    const l = ((locale ?? 'en').toLowerCase().split('-')[0]);
    if (SCAFFOLD_LOCALES.includes(l) &&
        typeof process !== 'undefined' &&
        process.env?.NODE_ENV !== 'production') {
        // eslint-disable-next-line no-console
        console.warn(`[@ummat/consent i18n-missing] locale="${l}" — falling back to en. Translate src/messages/${l}.json.`);
    }
