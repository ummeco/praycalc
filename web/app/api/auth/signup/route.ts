...
// Server-side proxy for Hasura Auth sign-up with Cloudflare Turnstile verification.
...
export async function POST(req: NextRequest) {
  ...
  const authRes = await fetch(`${AUTH_URL}/signup/email-password`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
      email: body.email,
      password: body.password,
      options: body.displayName ? { displayName: body.displayName } : undefined,
    }),
  }).catch(() => null)
