...
export function getTimes(params) {
    const { date, lat, lng, tz, elevation = 0, temperature = 15, pressure = 1013.25, asrConvention = AsrConvention.shafii, hanafiAngles = false, highLatitudeRule = HighLatitudeRule.none, shafaqMode = ShafaqMode.general, } = params;
    ...
    const jd = toJulianDate(new Date(Date.UTC(date.getUTCFullYear(), date.getUTCMonth(), date.getUTCDate(), 12, 0, 0)));
    ...
    if (highLatitudeRule !== HighLatitudeRule.none) {
        const adjusted = applyHighLatitudeRule(highLatitudeRule, fajrTime, ishaTime, isFinite(sunriseTime) ? sunriseTime : NaN, isFinite(maghribTime) ? maghribTime : NaN, isFinite(noonTime) ? noonTime : NaN, tw);
