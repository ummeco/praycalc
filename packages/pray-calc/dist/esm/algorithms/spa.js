...
function limitDegrees180pm(deg) {
    deg /= 360;
    let limited = 360 * (deg - Math.floor(deg));
    if (limited < -180)
        limited += 360;
...
function geocentricRightAscension(lamda, epsilon, beta) {
    ...
    return limitDegrees(rad2deg(Math.atan2(Math.sin(lambdaRad) * Math.cos(epsilonRad) - Math.tan(deg2rad(beta)) * Math.sin(epsilonRad), Math.cos(lambdaRad))));
}
...
function rightAscensionParallaxAndTopocentricDec(latitude, elevation, xi, h, delta, out) {
    ...
    out.deltaPrime = rad2deg(Math.atan2((Math.sin(deltaRad) - y * Math.sin(xiRad)) * Math.cos(deltaAlphaRad), Math.cos(deltaRad) - x * Math.sin(xiRad) * Math.cos(hRad)));
...
function atmosphericRefractionCorrection(pressure, temperature, atmosRefract, e0) {
    let delE = 0;
...
function julianEphemerisDay(jd, deltaT) { return jd + deltaT / 86400; }
function julianEphemerisCentury(jde) { return (jde - 2451545) / 36525; }
function julianEphemerisMillennium(jce) { return jce / 10; }
...
function nutationLongitudeAndObliquity(jce, x, spa) {
...
function greenwichMeanSiderealTime(jd, jc) {
    return limitDegrees(280.46061837 + 360.98564736629 * (jd - 2451545) + jc * jc * (0.000387933 - jc / 38710000));
}
...
function eot(m, alpha, delPsi, epsilon) {
...
function sunHourAngleAtRiseSet(latitude, deltaZero, h0Prime) {
    ...
    const argument = (Math.sin(deg2rad(h0Prime)) - Math.sin(latRad) * Math.sin(deltaZeroRad)) /
...
function calculateGeocentricSunRightAscensionAndDeclination(spa) {
...
function calculateEotAndSunRiseTransitSet(spa) {
    ...
    sunRts.jd = julianDay(sunRts.year, sunRts.month, sunRts.day, sunRts.hour, sunRts.minute, sunRts.second, sunRts.deltaUt1, sunRts.timezone);
    ...
    if (h0 >= 0) {
        ...
        const nuRts = [0, 0, 0];
        const alphaPrime = [0, 0, 0];
        const deltaPrime = [0, 0, 0];
        const hPrime = [0, 0, 0];
        const hRts = [0, 0, 0];
        for (let i = 0; i < 3; i++) {
        ...
        spa.sunrise = dayfracToLocalHr(sunRiseAndSet(mRts, hRts, deltaPrime, spa.latitude, hPrime, h0Prime, 1), spa.timezone);
        spa.sunset = dayfracToLocalHr(sunRiseAndSet(mRts, hRts, deltaPrime, spa.latitude, hPrime, h0Prime, 2), spa.timezone);
    }
    else {
...
function spaCalculate(spa) {
    ...
    if (spa.temperature <= -273 || spa.temperature > 6000)
        return 13;
    if (spa.deltaUt1 <= -1 || spa.deltaUt1 >= 1)
        return 17;
    ...
    rightAscensionParallaxAndTopocentricDec(spa.latitude, spa.elevation, spa.xi, spa.h, spa.delta, out);
    ...
    spa.delE = atmosphericRefractionCorrection(spa.pressure, spa.temperature, spa.atmosRefract, spa.e0);
 ...
 */
export function getSpa(date, latitude, longitude, timezone, options = {}) {
    const { elevation = 0, pressure = 1013, temperature = 15, deltaUt1 = 0, deltaT = 67, slope = 0, azmRotation = 0, atmosRefract = 0.5667, customAngles = [], } = options;
    const d = newState();
