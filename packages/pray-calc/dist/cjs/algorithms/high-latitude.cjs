...
exports.applyHighLatitudeRule = applyHighLatitudeRule;
 ...
 */
function applyHighLatitudeRule(rule, fajr, isha, sunrise, sunset, solarNoon, angles) {
    if (rule === index_js_1.HighLatitudeRule.none)
        return { fajr, isha };
    ...
    let offset;
    switch (rule) {
        case index_js_1.HighLatitudeRule.angleBased:
            // Use twilight angle as fraction: angle / 60 × night
            offset = (Math.min(angles.fajrAngle, angles.ishaAngle) / 60) * nightLength;
            break;
        case index_js_1.HighLatitudeRule.oneSeventh:
            offset = nightLength / 7;
            break;
        case index_js_1.HighLatitudeRule.middleOfNight:
