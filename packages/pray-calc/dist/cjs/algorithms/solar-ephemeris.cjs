...
exports.solarEphemeris = solarEphemeris;
exports.solarVerticalSpeed = solarVerticalSpeed;
exports.atmosphericRefraction = atmosphericRefraction;
exports.clamp = clamp;
/**
...
function solarEphemeris(jd) {
    ...
    const l0 = positiveMod(280.46646 + 36000.76983 * t + 0.0003032 * t * t, 360);
    // Mean anomaly M (degrees)
    const mDeg = positiveMod(357.52911 + 35999.05029 * t - 0.0001537 * t * t, 360);
    const mRad = mDeg * DEG;
 ...
 */
function solarVerticalSpeed(latRad, declRad, hAngleRad) {
    return (15 *
        Math.abs(Math.cos(latRad) * Math.cos(declRad) * Math.sin(hAngleRad)));
}
/**
...
function atmosphericRefraction(altitudeDeg, pressureMbar = 1013.25, temperatureC = 15) {
    ...
    const r0 = 1.02 /
        Math.tan((altitudeDeg + 10.3 / (altitudeDeg + 5.11)) * DEG);
