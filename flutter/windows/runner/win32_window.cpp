...
/// Registry key for app-level DPI setting.
...
void Win32Window::SetQuitOnClose(bool quit) {
  quit_on_close_ = quit;
}
...
LRESULT CALLBACK Win32Window::WndProc(HWND const window, UINT const message,
  ...
  if (message == WM_NCCREATE) {
    ...
    switch (message) {
      case WM_SIZE: {
        RECT rect = that->GetClientArea();
        if (that->child_content_ != nullptr) {
          // Size and position the child window.
          MoveWindow(that->child_content_, rect.left, rect.top,
      ...
      case WM_DPICHANGED: {
        ...
        SetWindowPos(window, nullptr, newRectSize->left, newRectSize->top,
